...
impl Shell for Elvish {
    fn activate(&self, exe: &Path, status: bool) -> String {
        ...
        if !is_dir_in_path(dir) {
            out.push_str(&format!("set paths = [ {dir} $@paths ]\n", dir = dir.display()));
        }
        // TODO: how to export a function from elvish eval?
        //
        //    fn rtx {{|@args|
        //      var command = ""
        //      if (> (count $args) 0) {{
        ...
        out.push_str(&formatdoc! {r#"
            ...
            set edit:before-readline = [ $@edit:before-readline $_rtx_hook~ ]
            _rtx_hook
            "#});
